...
shinyServer(
    function(input, output) {
        ...
        output$uiy <- renderUI({
            availableVars <- names(iris)[which(!names(iris)[1:4] %in% input$xaxis)]
            return(selectInput("yaxis", "Choose y-axis",
                               choices = availableVars,
                               selected = availableVars[1]))
        })
        
        output$uiy2 <- renderUI({
            availableVars <- names(iris)[which(!names(iris)[1:4] %in% input$xaxis2)]
        ...
        output$warnMsg <- renderText({
            if(length(input$coVars) < 2){
                '<b style="color:red">Choose at least two covariates</b>'
            }
        })
